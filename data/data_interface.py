...
class DInterface(pl.LightningDataModule):
    ...
    def setup(self, stage=None):
        ...
        if stage == 'fit' or stage is None or stage == 'validate':
            self.trainset = self.instancialize(phrase='train', audio_embedding=True)
            self.valset = self.instancialize(phrase='val', audio_embedding=True)
            

        # Assign test dataset for use in dataloader(s)
        if stage == 'test' or stage is None:
            
            self.testset = self.instancialize(phrase='val', audio_embedding=True)
    ...
    def train_dataloader(self):
        ...
        # return DataLoader(self.trainset, batch_size=self.batch_size, num_workers=self.num_workers, shuffle=True, multiprocessing_context='spawn')

    def val_dataloader(self):
        return DataLoader(self.valset, batch_size=self.batch_size, num_workers=self.num_workers, shuffle=False)
        # return DataLoader(self.valset, batch_size=self.batch_size, num_workers=self.num_workers, shuffle=False, multiprocessing_context='spawn')

    def test_dataloader(self):
        # return DataLoader(self.testset, batch_size=self.batch_size, num_workers=self.num_workers, shuffle=False)
        return DataLoader(self.testset, batch_size=1, num_workers=self.num_workers, shuffle=False)
